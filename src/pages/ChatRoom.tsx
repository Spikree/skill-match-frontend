...
const ChatRoom = () => {
  ...
  const userId = currentUser || "6790eca80f5ad123cbae35bd";
  ...
  const fetchMessages = async () => {
    ...
    setLoading(true);
    try {
      const response = await axiosInstance.get(`/message/getMessages/${chatId}`);
  ...
  const sendMessage = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axiosInstance.post(`/message/sendMessage/${chatId}`, { text });
